...
dataType
    : dataTypeName_ dataTypeLength? charsetClause_? collateClause_? UNSIGNED? ZEROFILL? | dataTypeName_ LP_ STRING_ (COMMA_ STRING_)* RP_ charsetClause_? collateClause_?
    ;

/**后续待调整，此处待整改 **/
dataTypeName_
    : CHAR | BINARY | DECIMAL | DOUBLE | INT | VARCHAR | BIGINT UNSIGNED | BIGINT | BINARY | BIT | BLOB | BOOL | DATE | DATETIME
    | DECIMAL | DOUBLE PRECISION | FLOAT | INT UNSIGNED | INTEGER | INTEGER UNSIGNED | LONG VARCHAR | LONGBLOB | LONGTEXT
    | MEDIUMBLOB | MEDIUMINT | MEDIUMINT UNSIGNED | MEDIUMTEXT | SMALLINT | SMALLINT UNSIGNED | TEXT | TIME | TIMESTAMP
    | TINYBLOB | TINYINT | TINYINT UNSIGNED | TINYTEXT | JSON
    ;
